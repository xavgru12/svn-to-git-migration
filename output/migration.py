...
def add_release_branch(data):
    ...
    complete_line_string = (f"branches = {data.remote_path}{optional_name_parts}/{{{last_part}}}"
...
def migrate_each_external(data):
    ...
    external_source_path = f"{configuration.local_path}/SharedComponentsSources/{data.name}"
    if not os.path.exists(external_source_path):
        os.makedirs(external_source_path)
    subprocess.check_output(["git", "svn", "init", configuration.base_remote_url],
                            cwd=external_source_path).decode(sys.stdout.encoding)
    ...
    if "NORImageCreator" != data.name:
        subprocess.check_output(["git", "svn", "fetch", "--quiet", "--quiet"], cwd=external_source_path,
                                creationflags=subprocess.REALTIME_PRIORITY_CLASS).decode(sys.stdout.encoding)
